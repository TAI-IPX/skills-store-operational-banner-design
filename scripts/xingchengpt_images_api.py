"""
xingchengpt_images_api.py — 标准 OpenAI Images API 封装（newapi.pro / XingchenGPT）
 ...
 用法：
   from xingchengpt_images_api import generate_image, edit_image, chat_completions_image
 ...
 CLI:
   py scripts/xingchengpt_images_api.py generate "A cat" -o output/cat.png
   py scripts/xingchengpt_images_api.py edit input/photo.png "Remove bg" -o output/edited.png
"""
...
BASE_URL = os.environ.get("XINGCHENGGPT_BASE_URL", "https://api.newapi.pro").rstrip("/") + "/v1"
...
def _get_api_key() -> str:
    ...
    if not key or not key.startswith("sk-"):
        raise RuntimeError(
            "XINGCHENGGPT_API_KEY 未设置或格式错误（需 sk- 开头）。"
            "请在 .env 或环境变量中配置。"
        )
    return key
...
def _get_proxies() -> dict | None:
    ...
    需要走代理时显式设置 XINGCHENGGPT_USE_PROXY=1 才会探测 HTTPS_PROXY / Windows 注册表代理。
    XINGCHENGGPT_NO_PROXY=1 仍受支持（向后兼容，效果与不设置任何代理开关一致）。
    """
    no_proxy = os.environ.get("XINGCHENGGPT_NO_PROXY", "").strip()
    if no_proxy.lower() in ("1", "true", "yes"):
        return None

    use_proxy = os.environ.get("XINGCHENGGPT_USE_PROXY", "").strip()
    ...
    if not img_url and not b64_out:
        for choice in data.get("choices", []):
            ...
            if isinstance(content, list):
                ...
                if mk:
                    img_url = mk.group(1)
                mk_data = re.search(r'!\[.*?\]\(data:image/[^;]+;base64,([^\s)]+)\)', content)
                if mk_data and not img_url:
                    b64_out = mk_data.group(1)
...
def generate_image(
    ...
    *,
    ...
    import requests as _requests
    api_key = _get_api_key()
    model = model or os.environ.get("XINGCHENGGPT_MODEL", DEFAULT_MODEL).strip()
    _size = size or os.environ.get("XINGCHENGGPT_SIZE", "auto").strip()
    _quality = quality or os.environ.get("XINGCHENGGPT_QUALITY", "auto").strip()
    ...
    print(f"[xingchengpt t2i] 生成图片（{model}，size={_size}，quality={_quality}）...", flush=True)
    ...
    if not img_url and not b64_out:
        print(f"[xingchengpt t2i] 未获取到图片。响应: {json.dumps(data, ensure_ascii=False)[:500]}", file=sys.stderr)
        return None
    ...
    if output_path:
        ...
        print(f"[xingchengpt t2i] 已保存: {out_path}", flush=True)
...
def edit_image(
    ...
    *,
    ...
    print(f"[xingchengpt edit] 编辑图片（{model}）...", flush=True)
    resp = _requests.post(
        url,
        files=files,
        data=data_fields,
    ...
    img_url, b64_out = _resolve_image_url(data)
    if not img_url and not b64_out:
        print(f"[xingchengpt edit] 未获取到图片。响应: {json.dumps(data, ensure_ascii=False)[:500]}", file=sys.stderr)
        return None
    ...
    if output_path:
        ...
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_bytes(img_bytes)
        print(f"[xingchengpt edit] 已保存: {out_path}", flush=True)
        return out_path.resolve()
    return img_bytes
...
def chat_completions_image(
    ...
    *,
    ...
    for ip in image_paths:
        p = Path(ip)
        if not p.is_file():
            print(f"[xingchengpt chat] 图片不存在: {p}", file=sys.stderr)
            continue
    ...
    print(f"[xingchengpt chat] 生成图片（{model}）...", flush=True)
    resp = _requests.post(url, json=body, headers=_get_headers(), timeout=600, proxies=_get_proxies())
    data = resp.json()

    img_url, b64_out = _resolve_image_url(data)
    if not img_url and not b64_out:
        print(f"[xingchengpt chat] 未获取到图片。响应: {json.dumps(data, ensure_ascii=False)[:500]}", file=sys.stderr)
        return None
    ...
    if output_path:
        ...
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_bytes(img_bytes)
        print(f"[xingchengpt chat] 已保存: {out_path}", flush=True)
        return out_path.resolve()
    return img_bytes


def main():
    parser = argparse.ArgumentParser(description="XingchenGPT Images API CLI")
