...
# SSL context — 每次使用时动态创建，支持运行时设置 LOVART_INSECURE_SSL=1
...
class AgentSkill:
    ...
    def __init__(
        ...
        # 代理支持：读取 HTTPS_PROXY/HTTP_PROXY 环境变量，构建带 ProxyHandler 的 opener
        _proxy = (
        ...
        _https_handler = urllib.request.HTTPSHandler(context=_make_ssl_ctx())
        if _proxy:
            ...
            self._opener = urllib.request.build_opener(_https_handler)
