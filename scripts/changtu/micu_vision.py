...
def call_micu_vision_with_images(
    ...
    *,
    thumb_max: int = 0,
    timeout: int = 120,
    block_session_on_fail: bool = True,
) -> str | None:
    """MICU 多图识图 + 文本回复。

    block_session_on_fail=False 时失败不置 _micu_vision_blocked，供艺术字 QA 多轮读字。
    """
    global _micu_vision_blocked
    load_dotenv()
    if _micu_vision_blocked and block_session_on_fail:
        return None
    ...
    for model in micu_vision_models():
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0:
                    time.sleep(2)
    if block_session_on_fail:
        mark_micu_vision_blocked("all_models_failed")
    return None
