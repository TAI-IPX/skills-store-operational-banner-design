...
def generate_image(
    ...
    *,
    ...
    """
    文生图：POST /v1/images/generations，多模型按序重试（MOXINGPT_MODEL 逗号分隔，最多3个）。
    output_path 为 None 时返回 bytes。
    max_retries: 下载失败时重试次数（CDN 不可达时重新调用 API）
    """
    import requests as _requests
    api_key = _get_api_key()
    _raw_models = model or os.environ.get("MOXINGPT_MODEL", DEFAULT_MODEL).strip()
    ...
    for mi, model_name in enumerate(model_list, 1):
        for attempt in range(max_retries):
            ...
            print(f"[moxingpt t2i] 生成图片（{model_name}，{mi}/{len(model_list)}，size={_size}，quality={_quality}）...", flush=True)
            ...
            if not img_url and not b64_out:
                ...
                print(f"[moxingpt t2i] model={model_name} 无可用图片（{mi}/{len(model_list)}）: {json.dumps(data, ensure_ascii=False)[:300]}", file=sys.stderr)
                if mi < len(model_list):
                    break  # 换下一模型
                return None
